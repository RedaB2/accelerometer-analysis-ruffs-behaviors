...
author:
  - Reda Boutayeb
  - Srujan Gowda Sathiganahally Jagadeesha
  - Rohit Milind Sonawane
output: 
  rmdformats::robobook:
    toc_depth: 2
    code_folding: hide
...
```{r Median-filter smoothing for sequential predictions}
smooth_pred_median <- function(df, k=5, group_cols=c('recording_id','segment_id'), order_col='window_start') {
    if(!('pred_beh' %in% names(df)) || !(order_col %in% names(df))) return(df)
    if(is.na(k) || k < 1) k <- 5
    if(k %% 2 == 0) k <- k + 1
    levs <- levels(as.factor(df$pred_beh))
    df$pred_beh <- factor(df$pred_beh, levels = levs)
    present_groups <- intersect(group_cols, names(df))
    if(length(present_groups) > 0){
        df <- df %>%
            group_by(across(all_of(present_groups))) %>%
            arrange(.data[[order_col]], .by_group = TRUE) %>%
            mutate(.pred_idx = as.integer(pred_beh),
                   .pred_idx_s = as.integer(round(stats::runmed(.pred_idx, k = k, endrule = "median"))),
                   .pred_idx_s = pmax(1L, pmin(length(levs), .pred_idx_s)),
                   pred_beh = factor(levs[.pred_idx_s], levels = levs)) %>%
            ungroup() %>%
            select(-.pred_idx, -.pred_idx_s)
    } else {
        df <- df %>%
...
```{r Apply median smoothing to HMM predictions}
hmm_cv_predictions <- smooth_pred_median(hmm_cv_predictions, k = 5)
hmm_test_predictions <- smooth_pred_median(hmm_test_predictions, k = 5)
```
...
```{r Apply median smoothing to RF CV predictions}
rf_predictions <- smooth_pred_median(rf_predictions, k = 5)
```
...
```{r Apply median smoothing to RF test predictions}
rf_test_predictions <- smooth_pred_median(rf_test_predictions, k = 5)
```

```{r Save smoothed RF test predictions}
write.csv(rf_test_predictions, 
          file=here(output_dir, glue("best_RF_test-predictions_{output_suffix}.csv")),
          row.names=FALSE)
```
...
```{r Apply median smoothing to NN predictions}
nn_predictions <- smooth_pred_median(nn_predictions, k = 5, group_cols=c('segment_id'), order_col='window_start_time')
```

```{r Save smoothed NN predictions}
