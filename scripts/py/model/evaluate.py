...
def evaluate_LSIOcv_scores(participant_scores, gen_gap_scores, input_cm, class_names, nb_subjects, filepath, filename, args):
    ...
    avg_acc = np.nanmean(participant_scores[0, :, :])
    std_acc = np.nanstd(participant_scores[0, :, :])
    avg_prc = np.nanmean(participant_scores[1, :, :])
    std_prc = np.nanstd(participant_scores[1, :, :])
    avg_rcll = np.nanmean(participant_scores[2, :, :])
    std_rcll = np.nanstd(participant_scores[2, :, :])
    avg_f1 = np.nanmean(participant_scores[3, :, :])
    std_f1 = np.nanstd(participant_scores[3, :, :])
    ...
    for i, class_name in enumerate(class_names):
        avg_acc = np.nanmean(participant_scores[0, i, :])
        std_acc = np.nanstd(participant_scores[0, i, :])
        avg_prc = np.nanmean(participant_scores[1, i, :])
        std_prc = np.nanstd(participant_scores[1, i, :])
        avg_rcll = np.nanmean(participant_scores[2, i, :])
        std_rcll = np.nanstd(participant_scores[2, i, :])
        avg_f1 = np.nanmean(participant_scores[3, i, :])
        std_f1 = np.nanstd(participant_scores[3, i, :])
    ...
    print('-------------------')
    print('Average results')
    avg_acc = np.nanmean(gen_gap_scores[0, :])
    std_acc = np.nanstd(gen_gap_scores[0, :])
    avg_prc = np.nanmean(gen_gap_scores[1, :])
    std_prc = np.nanstd(gen_gap_scores[1, :])
    avg_rcll = np.nanmean(gen_gap_scores[2, :])
    std_rcll = np.nanstd(gen_gap_scores[2, :])
    avg_f1 = np.nanmean(gen_gap_scores[3, :])
    std_f1 = np.nanstd(gen_gap_scores[3, :])
    print('Avg. Accuracy {:.4f} (±{:.4f}), '.format(avg_acc, std_acc),
          'Avg. Precision {:.4f} (±{:.4f}), '.format(avg_prc, std_prc),
          'Avg. Recall {:.4f} (±{:.4f}), '.format(avg_rcll, std_rcll),
          'Avg. F1-Score {:.4f} (±{:.4f})'.format(avg_f1, std_f1))
