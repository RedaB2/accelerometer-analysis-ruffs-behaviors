...
def train(train_features, train_labels, val_features, val_labels, network, optimizer, loss, config, lr_scheduler=None):
    """
    ...
    :param loss: loss object
        Loss object
    :param config: dict
        Config object which contains all training and hyperparameter settings
    ...
    if config['weighted']:
        ...
        # Plug back in any classes missing from this training fold with arbitrary weight, so that cross-entropy doesn't mess up
        classes = np.arange(config['nb_classes']) # all classes
        unique, counts = np.unique(train_labels, return_counts = True) # Classes in this training fold
        missing_idx = np.where(np.logical_not(np.isin(classes, unique)))[0]
        for idx in missing_idx: 
            class_weights = np.insert(class_weights, idx, 1)

        loss.weight = class_weights.to(config['gpu'])
...
def predict(test_features, test_labels, network, config):
    """
